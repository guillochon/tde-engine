...
\begin{abstract} 
Tidal disruption events (TDEs) appear to prefer host galaxies undergoing, or recovering from, a burst of star formation, implying that a rare minority of galaxy types produces most events. The required per-galaxy rates, as high as $10^{-2}$ gal$^{-1}$ yr$^{-1}$, are hard to achieve through stellar relaxation alone. Molecular clouds surrounding nuclear clusters help set the relaxation rate, offering a path to higher rates. We show that (post-)starburst galaxies, whose nearby prototypes contain large molecular gas reservoirs, are likely in a self-sustaining cycle: an enhanced disruption rate increases the density of the surrounding clouds via momentum injected by the unbound debris. Solving for the steady state, the rate saturates at $3\times10^{-3}$--$4\times10^{-2}(M_{\rm h}/10^{6}M_\odot)^{-1/6}$ yr$^{-1}$, tens to hundreds of times the canonical rate, the range reflecting whether the clouds form a spherical belt or a flattened disk. Either way, the black hole is surrounded by parsec-scale clouds of density $10^{4}$--$10^{5}$ cm$^{-3}$ resembling those of the Milky Way's Central Molecular Zone, totalling a few $\times10^{5}M_\odot$, comparable to the molecular disks now resolved around nearby active nuclei. The state is self-limiting: stellar collisions cap the rate, and the engine consumes its fuel in $10^{8}$--$10^{9}$ yr, comparable to the ages of post-starburst hosts. Because the equilibrium nucleus is heavily obscured ($A_{\rm V}\sim50$), we predict that many such nuclei are missing from optically selected samples and should instead be found in the infrared, and that surveys should target galaxies with central reservoirs of dense molecular gas and low-level AGN activity. Our rates also fall within the range required if the ``little red dots'' seen by {\it JWST} are recurring TDEs in dense nuclear clusters, supplying a mechanism for a rate those models have so far had to assume.
\end{abstract}
