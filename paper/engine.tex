...
\begin{align}
L_{\rm UDR}  &= 6 \times 10^{40} \Gamma_{-2} \mhsix^{1/3}~{\rm ergs~s}^{-1}\\
...
Condition~(\ref{eq:cond3}) is an energy integral rather than a monomial, so we first replace $\Gamma_{\rm TDE}(\sigma, M_{\rm h})$ with a power-law fit to that integral over $3 \times 10^{4}$--$3 \times 10^{8}~\msun$. The three conditions are then each monomial in the four unknowns, and the system can be solved in closed form by taking logarithms; the solution is a set of power laws in $M_{\rm h}$, which we quote below to two significant figures. For the nuclear cluster we find
\begin{align}
...
\begin{equation}
M_{\rm disk} = f_{\Omega} M_{\rm belt} = 1.3 \times 10^{5} \left(\frac{f_{\Omega}}{0.2}\right)\mhsix^{1.57}~\msun.
\end{equation}
Flattening therefore reduces the gas mass by another factor of $f_{\Omega}$, from the isotropic belt's $6.7 \times 10^{5}~\msun$ to $\sim 1 \times 10^{5}~\msun$ at $f_{\Omega} = 0.2$, below the median molecular disk mass measured by \citet{GarciaBurillo:2021a}, whose sample has a median of $\sim 6 \times 10^{5}~\msun$.
...
\begin{equation}
f_{\ast} \gtrsim 0.12 \quad\quad (p_{\rm TDE} = p_{\rm AGN}).\label{eq:fagn}
\end{equation}
...
\acknowledgements
\begin{sloppypar}
We thank Fabio~Antonini, Blakesley~Burkhart, Xian~Chen, Charlie~Conroy, Robert~Fisher, Nathan~Goldbaum, Luke~Zoltan-Kelley, Morgan~MacLeod, Peter~Maksym, Ilya~Mandel, Michael~McCourt, Smadar~Naoz, Ryan~O'Leary, Enrico~Ramirez-Ruiz, Lorenzo~Sironi, and Nicholas~Stone for thoughtful discussions. This work was supported in part by Einstein grant PF3-140108 (J.~G.). J.~G. would like to thank the Aspen Center for Physics (NSF Grant \#1066293) for their generous hospitality. First paper draft was originally completed circa 2018; paper was updated in 2026 with the assistance of the Anthropic AI models Claude Fable~5 and Opus~5, and X.ai model Grok 4.6. We thank J. Xavier Prochaska for providing access to AI models that were used to conduct independent AI reviews. All references and AI edits were hand-checked by the authors.\footnote{The manuscript source, the bibliography, and the scripts that solve the equilibrium system and generate the figures are available at \url{https://github.com/guillochon/tde-engine}.}
\end{sloppypar}
