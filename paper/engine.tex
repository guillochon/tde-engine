...
\begin{abstract} 
Tidal disruption events (TDEs) appear to prefer host galaxies undergoing, or recovering from, a burst of star formation, implying that a rare minority of galaxy types produces most events. The required per-galaxy rates, as high as $10^{-2}$ gal$^{-1}$ yr$^{-1}$, are hard to achieve through stellar relaxation alone. Molecular clouds surrounding nuclear clusters help set the relaxation rate, offering a path to higher rates. We show that (post-)starburst galaxies, whose nearby prototypes contain significant molecular gas reservoirs, are likely in a self-sustaining cycle: an enhanced disruption rate increases the density of the surrounding clouds via momentum injection from the unbound debris of disruptions. Solving for the steady state, the rate saturates at $3\times10^{-3}$--$4\times10^{-2}(M_{\rm h}/10^{6}M_\odot)^{-1/6}$ yr$^{-1}$, tens to hundreds of times the canonical rate, the range reflecting whether the clouds form a spherical belt or a flattened disk. Either way, the black hole is surrounded by parsec-scale clouds of density $10^{4}$--$10^{5}$ cm$^{-3}$ closely resembling those of the Milky Way's Central Molecular Zone, with a total mass of a few $\times10^{5}M_\odot$, comparable to the molecular disks now resolved around nearby active nuclei. The state is self-limiting: stellar collisions cap the rate, and the engine consumes its fuel on a timescale of $10^{8}$--$10^{9}$ yr, comparable to the ages of post-starburst hosts. Because the equilibrium nucleus is heavily obscured ($A_{\rm V}\sim50$), we predict that many nuclei are systematically missing from optically selected samples and should instead be found in the infrared, and that surveys should target galaxies with central reservoirs of the densest molecular gas accompanied by low levels of AGN activity. Finally, our equilibrium rates fall within the range required if the ``little red dots'' seen by {\it JWST} are recurring tidal disruptions in dense nuclear clusters, supplying a mechanism that sets a rate those models have so far had to assume.
\end{abstract}
...
\begin{figure*}[t]
\centering\includegraphics[width=0.8\linewidth,clip=true]{engine-diagram.pdf}
...
\begin{figure}[t]
\centering\includegraphics[width=0.9\linewidth,clip=true]{mom-rates.pdf}
...
For the disruption rate we take the full-loss-cone (pinhole) rate, appropriate when the angular momentum a star receives per orbit exceeds the width of the loss cone. Massive perturbers keep the loss cone full in exactly this way \citep{Perets:2007a}, and the belt clouds derived above are massive perturbers of the required magnitude. Following \citet{Syer:1999a} and \citet{Stone:2016a},
\begin{equation}
\begin{split}
\Gamma_{\rm TDE} = 4 \times 10^{-5} \mhsix^{4/3} & \left(\frac{n_{0}}{10^{5}~{\rm pc}^{-3}}\right) \\
& \times \left(\frac{100~{\rm km~s}^{-1}}{\sigma_{\rm h}}\right)^{3}~{\rm yr}^{-1},
\end{split}
\label{eq:gtde}
\end{equation}
which is equivalent to $\Gamma_{\rm TDE} \propto n_{0} r_{\rm t} G M_{\rm h}/\sigma_{\rm h}$ up to the numerical prefactor.

Equations~(\ref{eq:gcoll}) and (\ref{eq:gtde}) both depend only on $\sigma$ once $M_{\rm h}$ is fixed, so Equation~(\ref{eq:cond3}) alone determines the compression of the cluster; the remaining two conditions then fix the properties of the belt.

\subsection{The Engine State}\label{subsec:engine-state}
...
\begin{align}
\sigma &= 4.0 \times 10^{2} \mhsix^{1/6}~{\rm km~s}^{-1}\\
a_{\rm h} &= 5.5 \times 10^{-2} \mhsix^{2/3}~{\rm pc}\\
n_{0} &= 2.4 \times 10^{9} \mhsix^{-1}~{\rm pc}^{-3}\\
P_{\rm h} &= 1.2 \times 10^{3} \mhsix^{1/2}~{\rm yr}\\
\tau_{\rm relax,\ast} &= 1.9 \times 10^{8} \mhsix^{3/2}~{\rm yr},
\end{align}
...
\begin{align}
r_{\rm b} &= 6.4 \mhsix^{0.66}~{\rm pc}\\
{\cal M} &= 37 \mhsix^{0.31}\\
R_{\rm MC} &= 1.3 \mhsix^{0.79}~{\rm pc}\\
n_{\rm MC} &= 8.0 \times 10^{4} \mhsix^{-0.97}~{\rm cm}^{-3}\\
\sigma_{\rm cl} &= 6.9 \mhsix^{0.31}~{\rm km~s}^{-1}\\
\tau_{\rm c} &= 3.6 \times 10^{5} \mhsix^{0.49}~{\rm yr}\\
M_{\rm MC} &= 2.3 \times 10^{4} \mhsix^{1.41}~\msun\\
N_{\rm MC} &= 1.0 \times 10^{2} \mhsix^{-0.27}\\
M_{\rm belt} &= 2.4 \times 10^{6} \mhsix^{1.14}~\msun,
\end{align}
corresponding to a gas surface density through the belt of $1.9 \times 10^{4}~\msun~{\rm pc}^{-2}$. The equilibrium disruption rate is
\begin{equation}
\Gamma_{\rm eq} = \Gamma_{\rm coll} = 3.7 \times 10^{-2} \mhsix^{-1/6}~{\rm yr}^{-1},\label{eq:eqrate}
\end{equation}
a factor of several hundred above the canonical $10^{-4}$~yr$^{-1}$, and with only a very weak (and now inverted) dependence on black hole mass. The size, radiative time, angular coverage, and collective luminosity of the UDRs are
\begin{align}
R_{\rm UDR} &= 0.11 \mhsix^{1/2}~{\rm pc}\\
\tau_{\rm rad} &= 49 \mhsix^{0.59}~{\rm yr}\\
f_{\rm UDR} &= 7.6 \times 10^{-5} \mhsix^{-0.32}\\
L_{\rm UDR} = L_{\rm MC} &= 2.1 \times 10^{41} \mhsix^{1/6}~{\rm erg~s}^{-1}.
\end{align}
...
\begin{align}
n_{\rm bg} &= 5.7 \times 10^{3} \mhsix^{-0.70}~{\rm cm}^{-3}\\
L_{\rm AGN}/L_{\rm Edd} &= 6.0 \times 10^{-2} f_{\rm B,-3} \mhsix^{-0.21}\\
A_{\rm V} &\simeq 51 \mhsix^{-0.04},
\end{align}
and finally the average feeding rate of the black hole from TDEs relative to accretion of the surrounding gas is
\begin{equation}
\langle \dot{M}_{\rm TDE} \rangle / \dot{M}_{\rm amb} = 3.5 f_{\rm B,-3}^{-1}\mhsix^{-0.96}.\label{eq:eqaccratio}
\end{equation}

Several features of this solution deserve comment. The equilibrium is remarkably insensitive to black hole mass: $\Gamma_{\rm eq}$ varies by only a factor of three across the four decades of $M_{\rm h}$ over which the engine can operate. The clouds that emerge are not exotic. With $n_{\rm MC} \sim 10^{5}~{\rm cm}^{-3}$, $\sigma_{\rm cl} \sim 7~{\rm km~s}^{-1}$, radii of order a parsec, and strongly suppressed star formation, they are close analogues of the clouds observed in the Milky Way's own Central Molecular Zone, of which G0.253+0.016 (``the Brick'') is the best-studied example \citep{Longmore:2013a,Kruijssen:2014a,Barnes:2017a}. The engine state, in other words, does not require gas in an unobserved configuration; it requires CMZ-like gas around a nucleus in which the disruption rate has already been raised.
...
We also note the assumptions that limit the range of validity of Equations (\ref{eq:eqrate})--(\ref{eq:eqaccratio}). The gravitational focusing that sets Equation~(\ref{eq:gcoll}) requires $\sigma_{\rm h} < v_{\rm esc,\ast}$, which fails for $M_{\rm h} \gtrsim 10^{8}~\msun$; above $10^{8.5}~\msun$ main-sequence stars are swallowed whole in any case. At the other end, the ratio of the per-orbit angular momentum kick to the loss cone width for cusp stars is $q_{\ast} = 0.14 \mhsix^{-2/3}$, placing the cusp near the boundary between the full and empty loss cone regimes rather than safely within the former. Equation~(\ref{eq:gtde}) is therefore an upper envelope on the disruption rate, and $\Gamma_{\rm eq}$ should be read as such. We note that the belt clouds cannot themselves torque cusp stars efficiently ($q_{\rm MC} \sim 10^{-8}$ for a star deep in the cusp, though $q_{\rm MC} \gg 1$ at the belt radius itself); the loss cone is more plausibly kept populated by stars formed in the belt clouds on low-angular-momentum orbits, which is the resupply channel implied by the association of TDEs with recent star formation.
...
\subsubsection{Why the spherical solution survives an isotropic cusp}
...
\begin{figure*}[t]
\centering\includegraphics[width=0.4\linewidth,clip=true]{dominance.pdf}
\centering\includegraphics[width=0.4\linewidth,clip=true]{domratio.pdf}
\caption{Conditions under which tidal disruptions dominate all other sources of feedback in galactic nuclei. In the left-hand plot, the gold and green regions show where the AGN and TDE feedback mechanisms dominate for a given tidal disruption rate $\Gamma_{\rm TDE}$ and black hole mass $M_{\rm h}$. Within the region where TDEs dominate feedback, the equilibrium disruption rate relation is represented by the blue lines: solid for the spherical solution of Equation (\ref{eq:eqrate}), dashed for the disk-fed solution of Section~\ref{subsec:disk} with $f_{\ast} = 0.2$, with the shaded band spanning the range $\Gamma_{\rm eq} \propto f_{\ast}^{3/2}$ allowed by the geometry. The right hand plot shows the ratio of momentum injection from TDEs $\dot{p}_{\rm TDE}$ to that from all other momentum injection processes, $\dot{p}_{\rm other}$. If a system occupies any position within the green region, it will be driven towards the equilibrium line by adjusting $\Gamma_{\rm TDE}$.}
\label{fig:dominance}
\end{figure*}
...
Taken together, these are the conditions the model needs, and they are observed to persist on precisely the timescale over which we find an engine can run. We would add one caution: the causal arrow is not established by any of this. A merger that sparks an engine also produces a k+A spectrum and a central gas reservoir independently, so the association of TDEs with k+A hosts is equally consistent with the engine being a consequence of the same event rather than a cause of the elevated rate. Distinguishing the two requires the nuclear observations described above, not host-galaxy demographics.
